...
import { Plus, Edit, Trash2, Eye, EyeOff, Search, Image, ChevronLeft, ChevronRight, Trash, Loader } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';
import { getPhotoGalleries, deletePhotoGallery, updatePhotoGallery } from '../../services/apiService';
import { STORAGE_URL } from '../../config/api';
import ConfirmModal from '../../components/ConfirmModal';
import Can from '../../components/Can';
import { usePermissions } from '../../hooks/usePermissions';

const PhotoGalleriesList = () => {
  ...
  const fetchGalleries = async () => {
    try {
      setIsLoading(true);
      const response = await getPhotoGalleries();
      setGalleries(response.data);
    } catch (error) {
      toast.error('Failed to fetch photo galleries');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleBulkDelete = async (ids) => {
    try {
      await Promise.all(ids.map(id => deletePhotoGallery(id)));
      toast.success(`${ids.length} photo gallery${ids.length !== 1 ? 'ies' : ''} deleted successfully!`);
      setSelectedItems([]);
      fetchGalleries();
    } catch (error) {
      toast.error('Failed to delete some photo galleries');
    }
  };
  ...
  const toggleStatus = async (gallery) => {
    try {
      ...
      await updatePhotoGallery(gallery.id, formData);
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-20 px-4">
      ...
      <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300 mb-2">No Photo Galleries Found</h3>
      <p className="text-gray-500 dark:text-gray-400 mb-6 text-center max-w-md">
        {searchTerm
          ? 'No photo galleries match your search criteria. Try adjusting your search terms.'
          : 'Get started by creating your first photo gallery. Click the button below to add a new gallery.'}
      </p>
      {!searchTerm && (
        <Can module="photo_galleries" action="create">
          <button
            onClick={() => navigate('/admin/dashboard/photo-galleries/create')}
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading photo galleries...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Photo Galleries</h1>
      ...
      {selectedItems.length > 0 && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <Can module="photo_galleries" action="delete">
              <button
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        {gallery.feature_image ? (
                          <img
                            src={getImageUrl(gallery.feature_image)}
                            alt={gallery.title}
                      ...
                      <td className="px-6 py-4">
                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                          {gallery.main_images?.length || 0} image{gallery.main_images?.length !== 1 ? 's' : ''}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Can module="photo_galleries" action="update">
                          <button
                        ...
                        {!hasPermission('photo_galleries', 'update') && (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <Can module="photo_galleries" action="update">
                            <button
                              onClick={() => navigate(`/admin/dashboard/photo-galleries/edit/${gallery.id}`)}
      ...
      <ConfirmModal
        ...
        message="Are you sure you want to delete this photo gallery? This action cannot be undone."
      />
      ...
      <ConfirmModal
        ...
        title="Delete Photo Galleries"
        message={`Are you sure you want to delete ${bulkDeleteConfirm.ids.length} photo gallery${bulkDeleteConfirm.ids.length !== 1 ? 'ies' : ''}? This action cannot be undone.`}
      />
    </div>
  );
};
